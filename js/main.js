...
loadJsonData("../jsons/users.json").then((data) => {
    ...
    userClone.querySelectorAll(".user-name-list").forEach((eachList) => {
      eachList.addEventListener("mouseover", (e) => {
        if (e.target.innerHTML == list.name) {
          ...
          document.querySelector(".company-name").innerHTML = list.company.name;
...
loadJsonData("../jsons/posts.json").then((data) => {
...
loadJsonData("../jsons/albums.json").then((data) => {
  ...
  albumData.forEach((album) => {
    ...
    albumClone.querySelector(".album-title").setAttribute("data-id", album.id);
